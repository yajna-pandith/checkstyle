...
public class WriteTagCheck
     ...
     * @param ast the the DetailAST to check tag for.
            ...
            for (int i = 0; i < comment.length; i++) {
                ...
                if (matcher.find()) {
                    ...
                    if (tagFormat == null || tagFormat.matcher(content).find()) {
                        logTag(ast.getLineNo() + i - comment.length,
                                ast.getColumnNo(), tag, content);
                    }
                    else {
     ...
     * @param col the column number where the violations was found
